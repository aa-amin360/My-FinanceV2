...
type DebtSetupGridProps = {
  onChange: (validDebts: { name: string; amount: number }[]) => void;
};

export default function DebtSetupGrid({ onChange }: DebtSetupGridProps) {
  const [debts, setDebts] = useState<EntryRow[]>([{ name: "", amount: "" }]);
  const [bulkText, setBulkText] = useState("");

  // Propagate validated debt logs back to the parent state whenever inputs change
  useEffect(() => {
  ...
  const handleBulkParse = () => {
    ...
    const currentActive = debts.filter((d) => d.name || d.amount);
    setDebts([...currentActive, ...parsed]);
    setBulkText("");
  };
    ...
    <div className="space-y-4">
      ...
      <div className="pt-2 border-t border-black/[0.05] dark:border-white/[0.05] space-y-2">
        <label className="text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider block">
          Paste Multiple Debts (Format: Name Amount)
        </label>
        <textarea
